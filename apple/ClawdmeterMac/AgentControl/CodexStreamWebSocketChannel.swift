...
// a paired iPhone using op `codex-stream-subscribe`. Subscribes to the
// relay's per-session PassthroughSubject; each event becomes a JSON
// text WS frame.
//
// Does NOT spawn the sidecar — caller (the /send handler or the X1
// compose-draft path) is responsible for `relay.ensureRunning()`.
// Subscribers can attach BEFORE the sidecar runs; the relay holds a
// pending subject until ensureRunning promotes it.
...
public final class CodexStreamWebSocketChannel: WSChannel {
    ...
    public func start() {
        codexStreamLogger.info("codex-stream-subscribe started for session \(self.session.id.uuidString, privacy: .public)")
        sendGreeting()
